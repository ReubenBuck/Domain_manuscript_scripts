...
ChromoChoice = "R"
joinChromoR <- c(list(`H1-hESC`[[ChromoChoice]]), list(HUVEC[[ChromoChoice]]), list(HepG2[[ChromoChoice]]), list(GM12878[[ChromoChoice]]), list(`HeLa-S3`[[ChromoChoice]]), list(K562[[ChromoChoice]]))
...
pdf(file = "plots/repliBoundry/HUVEC_TEs_and_Segments_Huge_attempt_2.pdf", onefile = TRUE)
for(TEs in names(joinRepChromatin)){
  TE_choice <- TEs
  matList <- c(list(earlyAnno[[TE_choice]]),list(lateAnno[[TE_choice]]))
  
  
  layout(matrix(1:(2 *2), nrow = 2),heights = c(1,1.5))
  rotateTEmat <- NULL
  for(i in 1:2){
    TEmat = matList[[i]]
  ...
  for(i in 1:2){
    ...
    yMax <- max(c(max(abs(repMean[TE_choice] - colMeans(rotateTEmat[[1]])) + repMean[TE_choice]),
                  max(abs(repMean[TE_choice] - colMeans(rotateTEmat[[1]])) + repMean[TE_choice]), 
                  repMean[TE_choice] + 5*repSD[TE_choice]))
    waveMax <- max(c(colMeans(WaveList[[1]]), colMeans(WaveList[[2]])))
    waveMin <- min(c(colMeans(WaveList[[1]]), colMeans(WaveList[[2]])))
    
    par(mar=c(0,5,5,5))
    plot(y , type = "l", xaxt = "n", xlab= "", main = paste(TE_choice, c("Early", "Late")[i]),col = 1 , ylim = c(repMean[TE_choice] - (yMax - repMean[TE_choice]),yMax), lwd = 3, ylab = "TE coverage per 10000bp")
    abline(v=ncol(TEmat)/2 + .5)
    abline(h = repMean[TE_choice])
    abline(h = repMean[TE_choice] + 2*repSD[TE_choice], lty = 2)
    abline(h = repMean[TE_choice] - 2*repSD[TE_choice], lty = 2)
